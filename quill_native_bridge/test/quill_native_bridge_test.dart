...
class _MockQuillNativeBridgePlatform extends base_mock.MockQuillNativeBridgePlatform with MockPlatformInterfaceMixin {}
...
void main() {
  ...
  group('isSupported', () {
    test('returns correct value based on platform implementation', () async {
      for (final isSupported in {true, false}) {
        ...
        when(mockQuillNativeBridgePlatform.isSupported(exampleFeature)).thenAnswer((_) async => isSupported);

        final result = await plugin.isSupported(exampleFeature);
        verify(mockQuillNativeBridgePlatform.isSupported(exampleFeature)).called(1);
        expect(result, isSupported);
      }
    });

    test('passes the $QuillNativeBridgeFeature correctly to the platform implementation', () async {
      for (final feature in QuillNativeBridgeFeature.values) {
        when(mockQuillNativeBridgePlatform.isSupported(any)).thenAnswer((_) async => false);
  ...
  test('isIOSSimulator returns correct value based on platform implementation', () async {
    for (final isSimulator in {true, false}) {
      when(mockQuillNativeBridgePlatform.isIOSSimulator()).thenAnswer((_) async => isSimulator);
  ...
  test('getClipboardHtml returns correct value based on platform implementation', () async {
    for (final html in {'<center></center>', '<html></html>'}) {
      when(mockQuillNativeBridgePlatform.getClipboardHtml()).thenAnswer((_) async => html);
  ...
  test('copyHtmlToClipboard passes the HTML correctly to the platform implementation', () async {
    const exampleHtml = '<body></body>';
    when(mockQuillNativeBridgePlatform.copyHtmlToClipboard(any)).thenAnswer((_) async {});

    await plugin.copyHtmlToClipboard(exampleHtml);
    verify(mockQuillNativeBridgePlatform.copyHtmlToClipboard(exampleHtml)).called(1);
  });
  ...
  group('isAppleSafari', () {
    ...
    test('delegates to isAppleSafari from the platform API', () {
      for (final isAppleSafariValue in {true, false}) {
        when(mockQuillNativeBridgePlatform.isAppleSafari()).thenReturn(isAppleSafariValue);
